...
for raw in range(6):
    for col in range(6):
        if ((raw!=0 and raw!=5) and col==0) or ((raw>2 and raw<5) and col==5) or ((raw==0 or raw ==5)and (col!=0 and col!=5)) or (raw==2 and (col>0 and col<5)):
            print_6[raw][col] = "6"

for raw in range(6):
    for col in range(6):
        if (raw==0) or (col==1 and raw==5) or (col==2 and raw==4) or (col==5 and raw==1) or (col==3 and raw==3) or (col==4 and raw==2):
            print_7[raw][col] = "7"
